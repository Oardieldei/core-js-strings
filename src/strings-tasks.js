...
/**
 * Calculate the    of character codes of the given string.
 *
 * @param {string} str - The input string.
 * @return {number} - The    of character codes of the string.
 *
 * @example
 *     OfCodes('My String') => 861 (77 + 121 + 32 + 83 + 116 + 114 + 105 + 110 + 103 = 861)
 *     OfCodes('12345') => 255 (49 + 50 + 51 + 52 + 53 = 255)
 *     OfCodes('') => 0
 *     OfCodes() => 0
 */
...
function countVowels(str) {
  const newArr = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  const resultArr = [...str].reduce(
    (acc, item) => (newArr.includes(item) ? acc + 1 : acc),
    0
  );
  return resultArr;
}
...
function isPalindrome(str) {
  const newArr = [...str]
    ...
    .join('')
    .toLowerCase();
  const resultArr = [...newArr].reverse().join('') === newArr;
  return resultArr;
}
...
function encodeToRot13(str) {
  const inArr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const outArr = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const ind = inArr.indexOf(str[i]);
    if (ind === -1) result += str[i];
    else result += outArr[ind];
  }
  return result;
}
